...
def ASRAF():
	...
	os.system('clear')
	print(logo)
	print(" ┏━[•] BD SIM CODE 777 444 443 666 400 255 420]")
	code = input(' ┗━[+] SELECT : ')
	doamin = ' BD Number id cloner [ONLY-OK] '
	print(' ┏━[•] EXAMPLE : 1000,5000,10000,15000,20000] ')
	limit = int(input(' ┗━[+] LIMIT : '))
	for nmbr in range(limit):
		love = ''.join(random.choice(string.digits) for _ in range(6))
		kodb = ''.join(random.choice(string.digits) for _ in range(6))
	...
	with ThreadPool(max_workers=60) as yaari:
		...
		print('┣━[•] TOTAL ID   :  '+tl)
		print(f'┣━[•] SIM CODE   : \033[1;92m {code} ')
